...
Currently building the **NEURO ecosystem** — an AI-native platform where every layer is designed to run locally and efficiently. [madOS](https://github.com/madkoding/mad-os) is an Arch-based Linux distribution optimized for limited resources (~300MB–1.9GB RAM), featuring Sway/Hyprland Wayland compositors, native Ollama & OpenCode AI integration, ZRAM/zstd compression, and fully automated ISO builds via GitHub Actions. [Fractal-Mind](https://github.com/madkoding/fractalmind) provides cognitive memory architecture with RAPTOR-style fractal graph indexing and HNSW vector search on SurrealDB. [Neuro Agent](https://github.com/madkoding/neuro-agent) delivers intelligent multi-provider LLM routing (Ollama, OpenAI, Anthropic, Groq) with AST-based code review, LRU context caching, and 219+ passing tests. [Tachikoma](https://github.com/madkoding/tachikoma) is a modular AI agent ecosystem combining GraphRAG memory, multi-model routing, and integrated tools including web search and command execution. Also the creator of [madTrackers](https://madtrackers.com) — a hardware/software solution for full-body VR tracking (SlimeVR/VRChat).
